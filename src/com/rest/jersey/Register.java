...
public class Register {
	...
	// HTTP Get Method
		...
		public String doLogin(@QueryParam("name") String name, @QueryParam("email") String email, @QueryParam("password") String pwd, @QueryParam("companyId") String companyId){
			String response = "";
			//System.out.println("Inside doLogin "+uname+"  "+pwd);
			int retCode = registerUser(name, email, pwd, companyId);
			if(retCode == 0){
		...
		private int registerUser(String name, String uname, String pwd, String companyId){
			System.out.println("Inside checkCredentials");
			int result = 3;
			if(Utility.isNotNull(uname) && Utility.isNotNull(pwd)){
				try {
					if(DBConnection.insertUser(name, uname, pwd, companyId)){
